...
Your task is to keep the session documentation up-to-date based on the latest work increment.
...
INSTRUCTIONS:
1. Analyze the transcript increment to understand what progress was made, what decisions were taken, or what technical details were discovered.
2. Decide if you need to CREATE a new document or UPDATE an existing one.
   - Prefer specialized files (e.g., 'research-auth.md', 'spec-api.md') over generic ones.
   - Do NOT update 'session-history.md' (handled by another hook).
3. If creating/updating a file, output the FULL content of that file.
4. You can output multiple files if needed.
5. Use the 'Write' or 'Edit' tools to actually save the files to the session folder.
6. Be concise but comprehensive in your documentation.
7. If the increment contains no significant information worth documenting, simply do nothing and say 'No updates needed'.

GOAL: Ensure that if a new developer takes over this session, they can understand the current state from these documents."
